...
from pivot_turning_inversion import *
import argparse
import logging
import math
import os
...
def gen_main(opt):
    ...
    for style_type in sorted(opt.style_types.split(',')):
        opt2.style_path = os.path.join(opt.style_path, style_type)
        
        for content_type in sorted(opt.content_types.split(',')):
            #save path 설정
            opt2.save_path = os.path.join(opt.save_path, f'style_{style_type}', f'content_{content_type}')
            opt2.content_path = os.path.join(opt.content_path, content_type)
            os.makedirs(opt2.save_path, exist_ok=True)

            args = multi_concept_inversion(opt2)
            #pivot_turning_inversion.inversion(args.output_dir, args.content_path, args.save_path, args.style_path, args.max_train_steps, args.cross_attention_injection_ratio, args.self_attention_injection_ratio, args.pretrained_model_name_or_path)
            inversion(args.output_dir, args.content_path, args.save_path, args.style_path, args.max_train_steps, args.cross_attention_injection_ratio, args.self_attention_injection_ratio, args.pretrained_model_name_or_path)
