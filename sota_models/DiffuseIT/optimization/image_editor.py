    ...
    def __init__(self, args) -> None:
        ...
        if self.args.use_ffhq:
            self.model.load_state_dict(
                torch.load(
                    "resources/models/DiffuseIT/ffhq_10m.pt",
                    map_location="cpu",
                )
            )
            ...
            self.model.load_state_dict(
            torch.load(
                "resources/models/DiffuseIT/256x256_diffusion_uncond.pt"
                if self.args.model_output_size == 256
                else "resources/models/DiffuseIT/512x512_diffusion.pt",
                map_location="cpu",
            )
        )
        ...
        with open("./sota_models/DiffuseIT/model_vit/config.yaml", "r") as ff:
