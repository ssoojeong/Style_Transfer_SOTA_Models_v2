...
def parse_args(**parser_kwargs):
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default=f'stylized_images/StyleID'
    )
    ...
    parser.add_argument('--model_config', type=str, default='resources/configs/StyleID/v1-inference.yaml', help='model config')
    parser.add_argument('--precomputed', type=str, default='sota_models/StyleID/precomputed_feats', help='save path for precomputed feature')
    parser.add_argument('--ckpt_path', type=str, default='resources/models/sd-v1-4/sd-v1-4.ckpt', help='model checkpoint')
...
def feat_comp_save(precomputed, style_img_path, model, sampler, device, uc, time_idx_dict, ddim_sampler_callback, opt):
    if isinstance(style_img_path, Path):
        image_type = style_img_path.parts[-3]
        style_type = style_img_path.parts[-2]
    else:
        image_type = style_img_path.split('/')[-3]
        style_type = style_img_path.split('/')[-2]
        
    sty_feat_path_root = os.path.join(precomputed, image_type, style_type)
    os.makedirs(sty_feat_path_root, exist_ok=True)
    
    #feat 학습
    init_sty = load_img(style_img_path).to(device)
    seed = -1
    sty_feat_name = os.path.join(sty_feat_path_root, os.path.basename(style_img_path).split('.')[0] + '_sty.pkl')
    sty_z_enc = None

    if len(sty_feat_path_root) > 0 and os.path.isfile(sty_feat_name):
        ...
        sty_z_enc, _ = sampler.encode_ddim(init_sty.clone(), num_steps=opt.ddim_inv_steps, unconditional_conditioning=uc, \
...
def gen_main(opt):
    ...
    model = load_model_from_config(model_config, f"{opt.ckpt_path}")
    ...
    for style_type in opt.style_types.split(','):
        style_path = os.path.join(opt.style_path, style_type)
        style_img_path = os.path.join(style_path, os.listdir(style_path)[0])
        
        #style feat 저장
        sty_feat, sty_z_enc = feat_comp_save(opt.precomputed, style_img_path, model, sampler, device, uc, time_idx_dict, ddim_sampler_callback, opt)
        ...
        for content_type in opt.content_types.split(','):
            ...
            cnt_feat_path_root = os.path.join(opt.precomputed, 'content', content_type)
            os.makedirs(cnt_feat_path_root, exist_ok=True)
            ...
            for content_img_path in content_paths:
                ...
                #content feat 저장
                cnt_feat, cnt_z_enc = feat_comp_save(opt.precomputed, content_img_path, model, sampler, device, uc, time_idx_dict, ddim_sampler_callback, opt)
                ...
                with torch.no_grad():
                    with precision_scope("cuda"):
                        with model.ema_scope():
                            ...
                            img.save(os.path.join(save_path, content_name))
