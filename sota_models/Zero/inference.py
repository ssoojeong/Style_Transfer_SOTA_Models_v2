...
NUM_DDIM_STEPS = 50
GUIDANCE_SCALE = 7.5
STABLE_DIFFUSION_MODEL_PATH = "resources/models/stable-diffusion-v1-4"
SEED = 9999
START_STEP = 5
END_STEP = 30
TOTAL_STEP = 30
...
def parse_args():
    ...
    parser.add_argument(
        "--content_types",
        type=str,
        default='test' #all #ffhq,mscoco2017
    )
    parser.add_argument(
        "--style_path",
        type=str,
    ...
    parser.add_argument(
        "--style_types", 
        type=str,
        default='animeBackgrounds_1_Dinosaur', #all
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default=f'stylized_images/Zero'
    )
...
def gen_main(args):
    ...
    for style_type in args.style_types.split(','):
        ...
        style_image = load_image(style_img_path, device)
        
        for content_type in args.content_types.split(','):
            content_path = os.path.join(args.content_path, content_type)
            content_paths = [os.path.join(content_path, f) for f in os.listdir(content_path)]
            ...
            for content_img_path in content_paths:
                content_image = load_image(content_img_path, device)
                source_prompt = ""
                target_prompt = ""
                prompts = [source_prompt, target_prompt]
                
                _, content_latent_list, x_t, uncond_embeddings = null_inversion.invert(
                    content_img_path, prompts, verbose=True
                )

                start_code_content = x_t.expand(len(prompts), -1, -1, -1)
                
                content_name = content_img_path.split('/')[-1]
                ...
                editor = ReweightCrossAttentionControl(
                    ...
                    content_img_name=content_img_path,
                )
                regiter_attention_editor_diffusers(model, editor)
